...
import { getFAFSAApplications } from "./graphql/queries";
import { updateFAFSAApplications } from "./graphql/mutations";
const client = generateClient();
export default function FAFSAApplicationsUpdateForm(props) {
  const {
    id: idProp,
    fAFSAApplications: fAFSAApplicationsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = fAFSAApplicationsRecord
      ? { ...initialValues, ...fAFSAApplicationsRecord }
      : initialValues;
    ...
    setEnrollmentDetails(cleanValues.enrollmentDetails);
    setDrtResults(cleanValues.drtResults);
    setErrors({});
  };
  const [fAFSAApplicationsRecord, setFAFSAApplicationsRecord] = React.useState(
    fAFSAApplicationsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getFAFSAApplications.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFAFSAApplications
        : fAFSAApplicationsModelProp;
      setFAFSAApplicationsRecord(record);
    };
    queryData();
  }, [idProp, fAFSAApplicationsModelProp]);
  React.useEffect(resetStateValues, [fAFSAApplicationsRecord]);
  const validations = {
    fafsaId: [],
    studentId: [],
    applicationDate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          schoolCodes: schoolCodes ?? null,
          enrollmentDetails: enrollmentDetails ?? null,
          drtResults: drtResults ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateFAFSAApplications.replaceAll("__typename", ""),
            variables: {
              input: {
                id: fAFSAApplicationsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "FAFSAApplicationsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Fafsa id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || fAFSAApplicationsModelProp)}
