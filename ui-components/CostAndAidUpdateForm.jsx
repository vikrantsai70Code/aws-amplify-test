...
import { getCostAndAid } from "./graphql/queries";
import { updateCostAndAid } from "./graphql/mutations";
const client = generateClient();
export default function CostAndAidUpdateForm(props) {
  const {
    id: idProp,
    costAndAid: costAndAidModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = costAndAidRecord
      ? { ...initialValues, ...costAndAidRecord }
      : initialValues;
    setCostAidId(cleanValues.costAidId);
    setFafsaId(cleanValues.fafsaId);
    setCostOfAttendance(cleanValues.costOfAttendance);
    setAidPackageDetails(cleanValues.aidPackageDetails);
    setErrors({});
  };
  const [costAndAidRecord, setCostAndAidRecord] =
    React.useState(costAndAidModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCostAndAid.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCostAndAid
        : costAndAidModelProp;
      setCostAndAidRecord(record);
    };
    queryData();
  }, [idProp, costAndAidModelProp]);
  React.useEffect(resetStateValues, [costAndAidRecord]);
  const validations = {
    costAidId: [],
    fafsaId: [],
    costOfAttendance: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          costAidId: costAidId ?? null,
          fafsaId: fafsaId ?? null,
          costOfAttendance: costOfAttendance ?? null,
          aidPackageDetails: aidPackageDetails ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateCostAndAid.replaceAll("__typename", ""),
            variables: {
              input: {
                id: costAndAidRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CostAndAidUpdateForm")}
      {...rest}
    >
      <TextField
        label="Cost aid id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || costAndAidModelProp)}
